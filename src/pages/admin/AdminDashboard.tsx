...
const AdminDashboard = () => {
  ...
  const mockRecentActivity = [
    { id: "1", action: "New patient registered", user: "John Doe", time: "2 min ago", type: "user" },
    { id: "2", action: "IoT device connected", user: "ESP8266-Device-42", time: "5 min ago", type: "device" },
    { id: "3", action: "ML alert triggered", user: "Anomaly Detection", time: "15 min ago", type: "alert" },
    { id: "4", action: "Doctor assigned patient", user: "Dr. Smith", time: "30 min ago", type: "user" },
    { id: "5", action: "Firebase sync completed", user: "System", time: "1 hour ago", type: "system" },
  ];
    ...
    <DashboardLayout role="admin">
      <div className="space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            <h1 className="font-heading text-2xl lg:text-3xl font-bold text-foreground">System Administration</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <MetricCard title="Total Users" value={totalUsers} unit="registered" icon={Users} trend="up" trendValue="+12" status="normal" />
              ...
              {mockUserBreakdown.map((item) => (
                <div key={item.role}>
                  ...
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div className={`h-full ${item.color} rounded-full`} style={{ width: `${(item.count / totalUsers) * 100}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-card rounded-2xl p-6 shadow-card">
            ...
            <div className="space-y-4">
              {mockRecentActivity.map((activity) => {
                ...
                return (
                  <div key={activity.id} className="flex items-start gap-4 p-3 rounded-xl hover:bg-muted/50">
